...
extension to compile. Every command shells to `ops` on `PATH` (fallback `$OPS_HOME/ops`), so the
...
1. Make sure `ops` is on your `PATH` (or export `OPS_HOME=~/ops`).
2. Raycast → *Extensions* → *Script Commands* → *Add Directories* → point it at this folder
   (`~/ops/frontends/raycast`).
3. The commands appear in Raycast root search: **Ops Capture**, **Ops Search**, **Ops Task Add**,
   **Ops Task List**, **Ops Status**.
...
| Script | Runs | Mode |
|---|---|---|
| `quick-capture.sh` | `ops capture <text>` | compact |
| `search.sh` | `ops search <q> --json` → top hit paths | fullOutput |
| `task-add.sh` | `ops task add <title>` | compact |
| `task-list.sh` | `ops task list` | fullOutput |
| `status-inline.sh` | `ops orient --line` | inline (30s refresh) |
