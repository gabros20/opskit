...
> surface (`ops <verb>`) is the only way anything *gets done*, identical for you in a
...
4. **One command surface for everyone.** Human, script, scheduler, and agent all run the
   *same* `ops <verb>` lines. No private agent API. This is the consistency guarantee.
...
it can do is discoverable from `ops.json`. Swapping agents = point the new one at the
...
~/work         # YOUR CODE. A plain directory tree (NOT a git repo itself). Every
               ...
               # The ops wiki holds *notes about* these projects and knows where
...
Markdown, code, configs, templates → `~/ops` or `~/work`. PDFs, images, design files,
recordings, archives, datasets → `~/files`, with a markdown note in the wiki linking to
it. `~/ops` stays plaintext-only by policy (a doctor check flags tracked binaries).

How the three relate:

- `~/ops` *references* `~/work` (project notes carry `repo:` and `remote:` frontmatter)
...
- `~/dotfiles` installs the tools and puts `~/ops/bin` on PATH; it owns zero business
  knowledge.
- Restore order on a fresh machine: dotfiles → ops → work (bulk re-clone from the
  project registry) → files (restic restore). Full procedure in §14.

Hard boundaries:

- **iCloud is walled off.** `~/ops`, `~/work`, `~/files`, `~/dotfiles` are never inside iCloud
...
Naming: lowercase (`~/ops`, `~/work`, `~/dotfiles`) — terminal-friendly, and Finder
doesn't care. No Johnny.Decimal number prefixes (cut from v2): numeric prefixes buy
Finder sort order but break links when renumbered, and neither `ops search` nor an agent
needs them. Use Finder sidebar favorites instead (§13).

---

## 3. The `~/ops` repo layout
...
├── ops                        # the dispatcher script (`ops <verb>`); symlinked onto PATH
├── ops.json                   # GENERATED capability manifest (committed; rebuilt by `ops index --manifest`)
├── .gitignore
│
...
│   ├── operate-ops/SKILL.md   # ← THE system skill: drives everything (full text §12.3)
...
│
├── inbox/                     # raw capture zone: dumped text, dropped files; `ops triage` empties it
│
...
| Cut | Why |
|---|---|
| `bin/ops-task`, `ops-wiki`, … (8 sub-binaries) | One dispatcher + verb folders does the same with one pattern to learn |
...
| cron + systemd adapters | This is a Mac; launchd only, `ops job run` as the universal fallback |
...
## 4. The command surface — `ops <verb>`
...
```
SYSTEM
  ops help [verb]           # list verbs / usage for one (rendered from ops.json)
  ops status                # repo states, active/waiting tasks, last close/backup/index
...
FLOW
  ...
  ops triage                # PROPOSE filing of inbox items into tasks/wiki; human approves
  ops start                 # daily start: today's journal note, active tasks, carry-forward
  ops close                 # daily close: summarize day into journal, flag uncommitted work
  ops week                  # weekly review: shipped/stalled, repo health, SOP candidates

KNOWLEDGE
  ops search "<query>"      # hybrid local search → ranked file#heading hits
  ops wiki <action>         # open <slug> | new <type> <name> | stale | orphans

TASKS
  ops task <action>         # list | add "<title>" | show <id> | move <id> <status> | done <id>
...
BUSINESS
  ops invoice <client>      # DRAFT invoice + accountant packet (reads tax-formula.md; never sends)

JOBS
  ops job <action>          # list | run <name> | apply   (render+load launchd plists)
```

Design rules for the surface:

- **Flat verbs, shallow subactions.** A verb may take one subaction word
  (`ops task add …`) but never nests deeper. Ten memorable verbs for daily life
...
- **A new verb costs one folder.** Drop `bin/<verb>/{run.sh,cmd.json}`, run
  `ops index --manifest`, done — instantly visible to `ops help`, every agent, and the
...
```sh
#!/usr/bin/env bash
# ops — the single entrypoint. ~/ops/bin on PATH via dotfiles.
set -euo pipefail
OPS="${OPS_HOME:-$HOME/ops}"
VERB="${1:-help}"; shift || true
[ "$VERB" = "help" ] && exec "$OPS/bin/lib/help.sh" "$@"
CMD="$OPS/bin/$VERB/run.sh"
[ -x "$CMD" ] || { echo "unknown verb: $VERB (try: ops help)" >&2; exit 1; }
"$OPS/bin/lib/guardrail.sh" "$VERB" "$@"      # enforce risk class (§6)
"$OPS/bin/lib/log.sh" "$VERB" "$@"            # append invocation to .logs/
exec "$CMD" "$@"
```
...
`ops.json`. That one generated file is the contract between the system and any driver:
`ops help` renders from it, agents learn the full surface from it, the scheduler reads
...
// bin/invoice/cmd.json
{
  ...
  "usage": "ops invoice <client> [period]",
  "args": [
...
| `safe_write` | writes only inside `~/ops`, reversible via git | `capture`, `start`, `close`, `task`, `new`, `index` | run; every change is a revertible git diff |
...
- **The wall is by path, not by trust.** Guardrail resolves every target path; anything
  escaping `~/ops` / registered `~/work` repos / `~/files` — or entering the
...
#   export OPS_AGENT="claude"     # set in dotfiles; or "codex", "opencode", "ollama", ...
run_agent() {
  local prompt="$1"; local tools="${2:-read-only}"   # caller declares the scope it needs
  case "${OPS_AGENT:-claude}" in
    ...
    ollama)   ollama run "${OPS_MODEL:-llama3.1}" "$prompt" ;;
    none)     return 2 ;;                         # explicit no-agent mode
    *)        echo "set OPS_AGENT" >&2; return 1 ;;
  esac
}
```
...
1. **Verbs that call an agent** (`triage`, `close` summaries, `invoice` prose): the
   *script* calls `run_agent` internally. The caller never knows. Every such verb has a
   pure-shell fallback (e.g. `ops close` without an agent assembles the day's git log +
...
2. **Agents that call verbs** (interactive Claude Code session, future supervisor): the
   agent reads the skill file and drives `ops <verb>` like any human. This is the main
...
`OPS_AGENT=codex` and configuring Codex's adapter are independent steps.
...
Finder, moved with `mv` or `ops task move`, no database required.
...
tasks/done/       finished; swept into done/<year>/ by `ops week`
```
...
- `ops start` lists `active/` + `waiting/`; `ops close` flags active tasks with no log
  entry today; `ops week` sweeps `done/`.
...
├── archive/<year>/                  # dead repos, git-bundled (see ops archive, §8.5)
...
`ops new project "<name>"` asks (or takes a `--kind`) and files accordingly. This is the
...
back to the system: every project has a note `~/ops/wiki/projects/<slug>.md` whose
frontmatter carries `repo:` (local path) and `remote:` (clone URL). That registry is what
makes `ops repo health` and `ops repo clone --all` possible — and it is how the whole of
...
`ops new project "<name>"` scaffolds all of this from `~/ops/templates/project-repo/`,
...
### 8.5 Archiving dead repos — `ops archive <project>`

A repo that's done shouldn't rot in `labs/` taking disk and attention, but deleting it
loses history. `ops archive`: strips build artifacts (`node_modules`, `dist`, …),
...
(§14). `ops week` surfaces `labs/` repos untouched for 90 days as archive candidates.
...
`ops search` indexes plaintext; binaries are invisible to it — unless they have a
...
read shadow notes.** The pipeline is `ops files ingest` — for each new file in `inbox/`
...
creates the shadow note → the next `ops index` makes it findable. No agent configured?
...
- **Backup: restic → a cloud bucket** (Backblaze B2 or any S3). Encrypted client-side
  ...
  registry (§15) runs `restic backup ~/files` nightly; `ops backup` checks snapshot
  freshness and yells if the last one is >48h old; `ops doctor` verifies the repo is
...
- **Remote access: Tailscale to your Mac.** SSH in, run `ops` verbs, browse `~/files`
...
|---|---|---|---|---|
| `~/Desktop`, `~/Downloads` | macOS (can't move them) | downloads, screenshots, AirDrops, received files | `ops files ingest` / `ops capture` | **yes — auto-swept** |
| `~/ops/inbox/` | system | text/notes destined for the knowledge base | `ops triage` | no (you empty it) |
...
you have a week to `ops files ingest` what matters into `~/files`, then a 60-day net
...
trigger, preconditions, steps, done-criteria, failure modes, and which `ops` verbs /
...
|---|---|---|---|
| 1 | `~/ops/AGENTS.md` | The **contract**: what this is, the absolute rules, where to go next. Read first, every time. ~1 page. | Rarely changes |
| 2 | `~/ops/skills/operate-ops/SKILL.md` | The **operating manual**: the map, the routing/filing decisions, how to traverse the wiki, every workflow. The driving license. | Changes as the system grows |
| 3 | `~/ops/ops.json` | The **capability truth**: every verb, its args, what it reads/writes, its risk. Generated. The agent never hardcodes verbs; it reads this. | Auto-generated |
| 4 | `~/ops/wiki/conventions.md` | The **normative conventions**: frontmatter, naming, linking — plus the `## Filing rules` the system learns over time (§10). | Grows with corrections |
...
how; ops.json says with which verbs; conventions.md says in what shape; the repo's
AGENTS.md governs once you're inside it.** Nothing is improvised around this chain.

### 12.2 `~/ops/AGENTS.md` (full text — the contract)
...
### 12.3 `~/ops/skills/operate-ops/SKILL.md` (full text — the manual)
...
`operate-ops/SKILL.md` to **two different agents** and confirm they file the same repo to
...
exact `ops` verbs it uses. The agent loads one only when its `description` matches the
task, so the always-on context stays just AGENTS.md + operate-ops. When you add a verb,
update `operate-ops/SKILL.md` §6 (the surface summary) and, if it changes how things are
...
(`AGENTS.md` + `skills/operate-ops/SKILL.md`), plus a thin per-agent adapter that points
each agent at it.** No content is duplicated — adapters are two symlinks, a one-line
bridge file, and a few config lines, all committed to the ops repo so they restore for
...
| Agent | Reads the contract as | Finds the skill via | Lock shell to `ops` | Adapter (committed in `~/ops`) |
|---|---|---|---|---|
| **Codex CLI** | `AGENTS.md` — **native** (run from `~/ops` or `--cd ~/ops`) | can't add skill dirs (#22869) → **symlink** | `config.toml`: `sandbox_mode="workspace-write"`, `approval_policy="on-request"` | `.codex/config.toml` + `.codex/skills → ../skills` |
| **Claude Code** | **`CLAUDE.md` only — NOT `AGENTS.md`** | `.claude/skills/` only | `settings.json`: `permissions.allow:["Bash(ops:*)"]`, deny rest | `CLAUDE.md` (bridge) + `.claude/settings.json` + `.claude/skills → ../skills` |
| **Hermes** | `AGENTS.md` — native, **but only if the gateway's cwd is `~/ops`** | configurable external skill dir | `approvals.mode: smart`; pre-approve `ops` | none in-repo; configured in `~/.hermes/` via `~/dotfiles` |
| **OpenClaw** | `AGENTS.md` — native (workspace = `~/ops`) | `<workspace>/skills/` — **native** | **`exec` is YOLO by default** → must set allowlist | none in-repo; `~/.openclaw/` + `exec-approvals.json` via `~/dotfiles` |

**The canonical-source rule.** `~/ops/skills/` is the only real copy of every skill.
Codex and Claude reach it by a committed symlink (`.codex/skills` and `.claude/skills`
both → `../skills`); Hermes is configured with `~/ops/skills` as an external skill dir;
OpenClaw finds it natively because its workspace *is* `~/ops`. Four discovery paths, one
...
```markdown
<!-- ~/ops/CLAUDE.md  — the entire file. Claude Code won't read AGENTS.md, so bridge it. -->
@AGENTS.md

Before any ops action, read and follow `skills/operate-ops/SKILL.md`.
Operate only through the `ops <verb>` surface it defines. Never invent a verb.
```

```toml
# ~/ops/.codex/config.toml  — adapter, not a second source of truth
sandbox_mode   = "workspace-write"   # writes confined to ~/ops; ~/files & repos added per task
approval_policy = "on-request"       # ask before anything outside the sandbox
# [profiles.ops] may layer model/approval choices; the AGENTS.md it reads is ~/ops/AGENTS.md
```

`.claude/settings.json` carries only `permissions` (allow `Bash(ops:*)` and the repo's
...
machine config, so they belong in **`~/dotfiles`** (chezmoi), not the ops repo: for
Hermes, set the gateway working directory to `~/ops` (`terminal.cwd`/`MESSAGING_CWD`) and
add `~/ops/skills` as an external skill dir; for OpenClaw, set
`agents.defaults.workspace: "~/ops"` and **tighten `exec`** (`tools.exec.security:
allowlist` + an `exec-approvals.json` permitting only the `ops` binary) — its ungated
...
must explicitly say *"load `operate-ops` before any ops action"* (the §12.2 AGENTS.md and
...
**`ops doctor` checks the wiring.** Doctor verifies the adapters resolve: `CLAUDE.md`
...
- **Claude Code** has no native daemon/Telegram — it's a coding tool. Use it as the
  *headless executor* an `ops` verb calls (`claude -p`, scoped `--allowedTools`), not as
...
- **OpenClaw**'s workspace is "default cwd, not a hard sandbox" — absolute paths can still
  escape it; the `ops`-only allowlist and `~/ops`/`~/files` path wall (§5) are what
...
covers all of them with no structural change to `~/ops` — at most a symlink and a config
block per agent.

| Agent | Reads contract as | Finds the skill via | Lock shell to `ops` | Adapter (committed in `~/ops`) |
|---|---|---|---|---|
| **opencode** | `AGENTS.md` — **native** (first-match beats `CLAUDE.md`; global `~/.config/opencode/AGENTS.md` too) | `skills/` + `instructions` glob in `opencode.json`; native agents/skills | `permission: {bash: ask}` + allow `ops`; tools filtered pre-model | `.opencode/` config (or none — uses `~/ops/AGENTS.md` directly) |
| **Cursor CLI** (`cursor-agent`) | `AGENTS.md` + `CLAUDE.md` + `.cursor/rules` — **native, all at root** | `.cursor/rules` (skills injected via rules) | `~/.cursor/cli-config.json` / `.cursor/cli.json`: `Shell(ops)` allow, deny rest | `.cursor/cli.json` (permissions) |
...
- **`AGENTS.md` is the safe bet.** opencode, Cursor, Grok, Droid, pi, and Antigravity all
  honor a project-root `AGENTS.md` natively. So `~/ops/AGENTS.md` *is* the portable
...
- **`.agents/skills/` is emerging as the cross-tool skills path** (Antigravity, pi, and
  ...
  Now every agent reaches the one canonical `~/ops/skills/` — Claude/Codex via their
  ...
  discovery paths.** All symlinks are committed and `ops doctor` checks them.
- **Two new guardrail watch-items:**
  - **pi has *no* permission system** — it runs with the user's full rights and only gates
    *project trust*, not commands. For an `ops`-only surface, run it `--no-shell` (then it
    can't call `ops` either — so pi is best as a *read/plan* driver), or wrap it in a
...
read/plan/“gardener-of-knowledge” driver rather than the one holding the keys to `ops`.
...
1. **Finder (manual).** Browse `~/ops`; open markdown anywhere; drag a file into
   `inbox/`; drag a task file between status folders. Nothing breaks — it's just files.
   Sidebar favorites: `~/ops`, `~/ops/tasks/active`, `~/ops/inbox`, `~/work/clients`.
2. **Terminal (manual).** `ops <verb>` for system operations; raw `git`/`rg`/`$EDITOR`
   for everything else. No intermediary. (A Raycast/Alfred hotkey that runs `ops capture`,
   `ops files ingest`, or `ops new` is just an ergonomic shortcut to this plane — an
   optional human launcher, not a fifth plane; it calls the same verbs.)
3. **Scheduler (automated, no judgment).** launchd fires the same `ops` lines for
...
|---|---|---|
| **git remotes** | `~/ops`, `~/dotfiles`, every `~/work` repo (incl. opted-in `tools/`/`labs/`) | `git push` |
| **restic → encrypted bucket** | `~/files` (versioned, off-site, remote-restorable) | nightly job (§9.3) |
| **iCloud** | irreplaceable personal/legal/family originals — passport, certificates, health, signed master contracts, baby/family docs | always synced; **walled off from every `ops` path** |
...
exec $SHELL -l                                               # reload so ~/ops/bin is on PATH
...
git clone <ops-remote> ~/ops
ops doctor --init            # creates ~/work + ~/files skeleton folders; verifies everything
ops repo clone --all         # re-clone every ~/work repo from the wiki registry
...
frontmatter (**the ops repo is the manifest of your work repos**, so `~/work` rebuilds
without ever being one repo), and **auth is established before any clone or restore** —
the single most common reason a "just git clone it" plan fails in practice. `ops doctor
...
### 14.3 `ops backup` (run weekly by a job; loud)

Checks ops + dotfiles for uncommitted/unpushed changes; walks the project registry and
...
`ops` on PATH · folder structure + templates intact · `ops.json` parses and matches
`bin/` · index rebuilds · job registry parses · remotes configured · restic repo
reachable · no secret-looking files tracked · no binaries tracked in `~/ops` · **agent
...
(idempotent). First command after any restore; also run by `ops week`.
...
(`ops job apply`); `ops job run <name>` runs anything manually (the universal fallback —
also the migration path if the host ever isn't a Mac).

```yaml
jobs:
  index:        { command: "ops index",          schedule: { interval_minutes: 60 }, risk: read }
  close_nudge:  { command: "ops close --automated", schedule: { daily: "18:30" },    risk: safe_write }
  backup_check: { command: "ops backup",         schedule: { weekly: "Fri 17:00" },  risk: read }
...
- **Daily (~5 min).** `ops capture` all day as thoughts arrive (zero-decision capture —
  routing is triage's job, not capture's). `ops start` in the morning; `ops close` at
  day's end (the 18:30 job nags if you forget).
- **Weekly (~20 min).** `ops week`: approve triage filings, scan `ops repo health` and
  `ops backup`, sweep done tasks, and answer the one compounding question: *what did I
...
- **Monthly (~30 min).** Review and send the drafted invoices + accountant packet for
  ...
  anything you no longer run? `ops archive` it.
- **As-needed.** `ops files ingest` from Desktop/Downloads when something worth keeping
  arrives; `ops archive` a lab repo the moment you know it's done.
- **Quarterly (~1 hr).** Prune unused verbs/skills (the logs say which), review `ops week`'s
...
anything mutating · absolute paths from `$OPS_HOME` · fail fast, loud, non-zero ·
...
1. **Roots + git + Finder.** Create `~/ops` per §3 (folders, README, AGENTS.md,
   conventions.md, templates), push to a private remote. Create `~/work` tree. Usable by
   hand immediately.
2. **Dispatcher + first pure-shell verbs.** `ops`, `bin/lib/`, then `capture`, `task`,
   `help`, `status`, `search` (FTS5), `index`. Put on PATH via dotfiles. *This phase is
   80% of daily value.*
3. **Manifest.** `cmd.json` sidecars + manifest build → `ops.json`. `ops help` and
...
5. **The agent entry point + adapters + agent test.** Write `AGENTS.md` (§12.2) and
   `operate-ops/SKILL.md` (§12.3) — especially the routing tree (§12.3 #4a) and the
...
7. **Work standards.** `templates/project-repo/`, `ops new`, `ops repo health/clone`,
   worktree policy. Retrofit AGENTS.md + script/* onto your 2–3 most active repos.
8. **Business + jobs.** `ops invoice` (reading tax-formula.md), `jobs/registry.yaml` +
   launchd adapter, `ops backup`, `ops doctor`.
...
- `~/ops` and `~/dotfiles` are git repos with remotes; `~/work` tree exists.
- The ~16 verbs of §4.1 work; `ops help` renders from `ops.json`.
...
- `AGENTS.md` + `operate-ops/SKILL.md` pass the two-agent agnosticism check: two
  different agents file the same repo to the same root, traverse the wiki the same way,
  and refuse the same actions.
- Per-agent adapters (§12.5) are committed and `ops doctor` confirms they resolve, so a
...
- You can find any note in <10 seconds via `ops search`, and any file by eye in Finder.
- `~/files` exists with restic backing it up nightly; `ops backup` notices a stale snapshot.
- Important documents are findable via `ops search` through their shadow notes.
...
|---|---|---|
| Topology: ops / work / dotfiles split | v1.0 | **Adopted** — solves code-repos-outside-the-system cleanly |
| Verb dispatcher + `cmd.json` → `ops.json` manifest | v2 | **Adopted** — capability discovery without hardcoding |
| Guardrails as per-verb risk in the dispatcher | v2 | **Adopted**; v1.0's `policy.yaml` **deleted** (duplicate truth) |
| Agent indirection (`OPS_AGENT` + `agent.sh`) | v2 | **Adopted** — exactly one swap point |
...
| Work-repo registry in wiki frontmatter → `ops repo clone --all` | new (implied by both) | **Adopted** — makes ~/work reconstructable |
...
| cron/systemd adapters | v1.0 | **Cut** — launchd + `ops job run` fallback |
...
| Shadow notes (`ops files ingest`) | v3.1 | **Added** — binaries become searchable via wiki notes, not asset trawling |
| restic → encrypted bucket for `~/files`; Tailscale for remote | v3.1 | **Added** — boring, proven; key in 1Password, never in a repo |
| `~/work` lifecycle: `tools/`, routing rule, `ops archive` (git-bundle) | Daddy Dev OS | **Added** — fills the repo birth→death story the design lacked |
...
| `~/work` + `~/files` skeleton via `ops doctor --init`; macOS `defaults` as chezmoi run-script | Automation doc (bootstrap) | **Added** — the empty folders + Finder prefs were in no repo (§14.2/§14.4) |
...
| Full bash script bodies, launchd XML, Raycast launcher files | Automation doc | **Rejected as core** — implementation, not design; they live in `~/ops/bin/` + `~/dotfiles`, generated/edited there, not pasted into the design |
...
| Canonical `~/ops/skills/` reached by 4 discovery paths (native/external-dir/symlink) | agent research | **Added** — keeps one source of truth while satisfying each agent's fixed skill location |
| Hermes/OpenClaw home-dir config in `~/dotfiles`, not the ops repo | agent research | **Added** — those configs are machine config (chezmoi); ops repo holds only the project-scoped Codex/Claude adapters |
