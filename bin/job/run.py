#!/usr/bin/env python3
"""
ops job list | run <name> | apply — the §15 scheduler-neutral jobs surface. Definitions live in
...
REGISTRY = paths.OPS_HOME / "jobs" / "registry.json"
...
def _load():
    if not REGISTRY.exists():
        print(f"no jobs registry at {REGISTRY.relative_to(paths.OPS_HOME)}", file=sys.stderr)
        sys.exit(1)
...
def _plist(name, job) -> str:
    toks = job["command"].split()
    args = [str(paths.OPS_HOME / "ops"), *toks[1:]] if toks and toks[0] == "ops" else toks
    ...
    log = paths.OPS_HOME / ".logs" / "jobs" / f"{name}.log"
    return (f'<?xml version="1.0" encoding="UTF-8"?>\n'
            f'<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" '
            f'"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
            f'<plist version="1.0">\n<dict>\n'
            f'  <key>Label</key>\n  <string>com.ops.{name}</string>\n'
            f'  <key>ProgramArguments</key>\n  <array>{pa}\n  </array>\n'
            f'  <key>EnvironmentVariables</key>\n  <dict>\n'
            f'    <key>OPS_HOME</key><string>{paths.OPS_HOME}</string>\n  </dict>\n'
            f'{when}\n'
...
def main(argv):
    ...
    if action == "list":
        ...
        r = subprocess.run(cmd, capture_output=True, text=True,
                           env={**__import__("os").environ, "OPS_HOME": str(paths.OPS_HOME)})
