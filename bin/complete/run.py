#!/usr/bin/env python3
"""
ops complete [<typed word>...] [--json] — the structured completion contract (ops.json/3).

Given the words already typed after `ops` (everything BEFORE the word being completed), emit the
candidates for the next word as structured rows `{value, description, kind}`. This is the clean,
documented sibling of the zsh helper `ops __complete` (which emits the same candidates as lossy
...
grammar from the contract instead of re-parsing usage strings. Risk `read`.
  ...
  ops complete                 -> the verb list
  ops complete task            -> task's subcommands (list/add/show/move/done)
  ops complete task move T-42  -> the status enum (active/waiting/…)
  ops complete wiki open       -> live note slugs
"""
import sys
