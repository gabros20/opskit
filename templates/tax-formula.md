...
The single source of truth for how `ops invoice` computes a draft. Edit these to your
jurisdiction. `ops invoice` reads the frontmatter above:
...
`ops invoice` only ever produces a DRAFT in the client's `~/files/.../out/` folder. It never
