...
NEVER loads a real model: OPS_IMAGE_FAKE short-circuits read_text/describe, and the OCR/VLM fallback
...
def _load_imagelib():
    ...
    spec = importlib.util.spec_from_file_location("ops_imagelib", REPO / "bin" / "lib" / "imagelib.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["ops_imagelib"] = mod
...
def main() -> int:
    il = _load_imagelib()
    for k in ("OPS_IMAGE_FAKE", "OPS_OCR", "OPS_VLM", "OPS_VLM_FALLBACK", "OPS_MLX", "OPS_VLM_KEEP_ALIVE"):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # ---------- OPS_IMAGE_FAKE: deterministic no-model seam ----------
        ...
        # OPS_OCR=none short-circuits before any probing — force a probe True + a raising runner to prove it
        ...
        try:
            ...
            text, backend = il.read_text(png)
            check("OCR OPS_OCR=none short-circuits (no probing/calls)", (text, backend) == ("", "none"),
                  str((text, backend)))
        finally:
        ...
        try:
            os.environ["OPS_VLM"] = "primary-model"
            os.environ["OPS_VLM_FALLBACK"] = "fallback-model"
            il._has_mlx = lambda: False
            ...
            os.environ.pop("OPS_VLM", None); os.environ.pop("OPS_VLM_FALLBACK", None)
            il._has_mlx, il._has_ollama, il._run_mlx_vlm, il._run_ollama_vlm = orig

        # OPS_VLM=none short-circuits before any probing/calls
        orig_ollama, orig_run = il._has_ollama, il._run_ollama_vlm
        try:
            il._has_ollama = lambda: True
            il._run_ollama_vlm = _boom
            os.environ["OPS_VLM"] = "none"
            cap, desc, backend = il.describe(png)
            check("VLM OPS_VLM=none short-circuits (no probing/calls)",
                  (cap, desc, backend) == ("", "", "none"), str((cap, desc, backend)))
        finally:
            os.environ.pop("OPS_VLM", None)
