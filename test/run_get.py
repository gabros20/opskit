...
where capture/search/week work. Fully offline — the clone source is the local repo (OPS_GET_REPO)."""
...
def run(args, target, extra=None):
    env = {**os.environ, "OPS_GET_REPO": str(REPO), "OPS_GET_DIR": str(target)}
    if extra:
        env.update(extra)
...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        check("demo clones a vault with the ops binary", (vault / "ops").exists())
        ...
        s = subprocess.run([str(vault / "ops"), "search", "demo", "--json"],
        ...
        w = subprocess.run([str(vault / "ops"), "week"], capture_output=True, text=True, env=env)
